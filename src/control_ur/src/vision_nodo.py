...
# --- IMPORTS DE ROS ---
import rospy
# CAMBIO 1: Importamos Pose en lugar de Float64MultiArray
...
class DiceDetectorPro:
    def __init__(self, cam_index=4, calib_file="ost.yaml"):
        ...
        # CAMBIO 2: El publisher ahora envía mensajes tipo Pose
        ...
        print("✅ Nodo ROS iniciado (Modo Pose: X, Y).")
                        ...
                        if solidity > 0.85:
                            ...
                            if 1 <= num <= 6:
                                ...
                                if should_send and r_xm is not None:
                                    # CAMBIO 3: Creación y envío del mensaje Pose
                                    msg = Pose()
                                    msg.position.x = r_xm
                                    msg.position.y = r_ym + 0.01
                                    msg.position.z = 0.0 # No necesario, lo maneja el robot
                                    
                                    # Importante: Enviar un cuaternión válido (identidad) 
                                    # para que MoveIt no se queje, aunque solo usemos X e Y.
                                    msg.orientation.w = 1.0 
                                    
                                    self.pub.publish(msg)
                                    rospy.loginfo(f"🚀 ENVIADO POSE: X={r_xm:.3f} Y={r_ym:.3f} | Dado: {num}")
