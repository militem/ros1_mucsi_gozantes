...
class DiceDetectorPro:
    def __init__(self, cam_index=0, calib_file="ost.yaml"):
        ...
        self.pub = rospy.Publisher('/ur_move_to_pose', Float64MultiArray, queue_size=10)
        
        # --- CONSTANTES DE MOVIMIENTO ROBOT ---
        self.ROBOT_Z_METERS = 0.25 
        self.ROBOT_RX = 3.1416      
        self.ROBOT_RY = 0.0
        self.ROBOT_RZ = 0.0
        
        # ============================================================
        # --- DIMENSIONES REALES (AJUSTA ESTO SI ES NECESARIO) ---
        # ============================================================
    ...
    def procesar_arucos_y_escala(self, img, gray):
        ...
        if ids is not None and len(ids) >= 4:
            ...
            if dist_px_vertical > 0:
                ...
                cv2.putText(img, "ORIGEN (0,0)", (tl[0], tl[1]-25), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0,0,255), 2)
    ...
    def configurar_sistema(self):
        ...
        while not rospy.is_shutdown():
            ...
            if key == 32: # Espacio
                ...
                rospy.signal_shutdown("Cancelado")
                break
    ...
    # =========================================================
    #  AQUÍ ESTÁ EL APAÑO PARA EL NÚMERO 6
    # =========================================================
    def analizar_puntos_v18(self, gray_full, contour, box):
        ...
        puntos_validos = 0
        puntos_feos = 0 # Contamos también los que no son perfectamente redondos
        ...
        for cd in cnts_dots:
            ...
            # Filtro de tamaño (generoso)
            if (die_area * 0.005) < area_dot < (die_area * 0.25): 
                perim = cv2.arcLength(cd, True)
                if perim == 0: continue
                circularity = 4 * np.pi * area_dot / (perim * perim)
                
                # 1. Puntos PERFECTOS (Verdes)
                if circularity > 0.40:
                    puntos_validos += 1
                    cv2.drawContours(debug_roi, [cd], -1, (0, 255, 0), -1)
                
                # 2. Puntos "FEOS" (Amarillos) - Solo para el caso del 6
                # Si es un poco deforme pero tiene tamaño de punto, lo contamos como posible
        ...
        # Si tenemos menos de 6 estrictos, pero sumando los "feos" llegamos a 6 o más...
        # Asumimos que es un 6 que se ve mal por perspectiva.
        if puntos_validos < 6 and (puntos_validos + puntos_feos) >= 6:
            resultado = 6
            # Opcional: Escribir en debug que se aplicó el fix
                        ...
                        if solidity > 0.85:
                            ...
                            if 1 <= num <= 6:
                                ...
                                if r_x_cm is not None:
                                    coord_text = f"X:{r_x_cm:.1f} Y:{r_y_cm:.1f} cm"
                                ...
                                if should_send and r_xm is not None:
                                    ...
                                    msg.data = [r_xm, r_ym, self.ROBOT_Z_METERS, self.ROBOT_RX, self.ROBOT_RY, self.ROBOT_RZ]
                                    self.pub.publish(msg)
                                    rospy.loginfo(f"🚀 ENVIADO: {num} | {coord_text}")
                                    self.click_point = None
